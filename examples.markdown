...
| Negative Multinomial $\\\\ (p \in [0,1)^d, y_0\in \mathbb{R}_{++},\\\\ p_0 := 1-\Sigma_{i=1}^d)$      |  $\sum_{i=0}^dy_i\log\left(\frac{y_i}{p_i \bar{y}}\right) \;\; (\bar{y}:= \sum_{i=0}^dy_i)$            |  $\mathbb{R}^d_{+} \cap I(p)$                    |
